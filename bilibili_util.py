...
class BilibiliClient:
    def __init__(self):
        ...
        self.canvasFp = "".join(
            [str(random.choice("0123456789abcdef")) for _ in range(32)]
        )
        self.webglFp = "".join(
            [str(random.choice("0123456789abcdef")) for _ in range(32)]
        )
    ...
    def _get_newest_version(self):
        ...
        try:
            resp = httpx.get(
                "https://app.bilibili.com/x/v2/version?mobi_app=android",
                headers=tmp_headers,
            ).json()
    ...
    def _hmac_sha256(self, key, message):
        return (
            hmac.new(key.encode("utf-8"), message.encode("utf-8"), hashlib.sha256)
            .digest()
            .hex()
        )
    ...
    def _init_buvid(self):
        ...
        fp_raw = (
            ...
            + "".join([str(random.choice("0123456789abcdef")) for _ in range(16)])
        )
    ...
    def _on_request(self, request: httpx.Request):
        ...
        self.session.cookies.update(
            {
                "identify": quote(
                    urlencode(self._app_sign({"ts": int(time.time() * 1000)}))
                ),
    ...
    def _app_sign(self, params: dict) -> dict:
        ...
        sign = hashlib.md5(
            (query + "560c52ccd288fed045859ed18bffd973").encode()
        ).hexdigest()
    ...
    def post(self, url: str, **kwargs) -> httpx.Response:
        ...
        if resp.status_code == 200:
            ...
            return {
                "code": 429,
                "message": f"请求被限流：[{resp.status_code}]",
                "data": None,
            }
            ...
            return {
                "code": 412,
                "message": f"请求被风控：[{resp.status_code}]",
                "data": None,
            }
        else:
            resp.read()
    ...
    def save(self):
        ...
        for i in list(self.session.cookies):
            ...
            if self._cookies[i] is None:
                self._cookies[i] = self.session.cookies.get(
                    i, domain="show.bilibili.com"
                )
            if self._cookies[i] is None:
